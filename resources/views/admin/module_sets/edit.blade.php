...
<div class="row mb-4">
    <div class="col-12">
        ...
        <h2 class="mb-0 text-gray-800" style="font-weight: 700;">Edit Module Portfolio</h2>
    ...
    <div class="col-md-8">
        <div class="card border-0 shadow-sm" style="border-radius: 15px;">
            <div class="card-body p-4">
                <form action="{{ route('admin.module-sets.update', $moduleSet) }}" method="POST">
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-4">
                        <label for="name" class="form-label fw-bold">Portfolio Name</label>
                        <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="e.g. Module 1" value="{{ old('name', $moduleSet->name) }}" required>
                        @error('name')
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="level_id" id="level_id" class="form-select @error('level_id') is-invalid @enderror" required>
                                ...
                                @foreach($levels as $level)
                                    <option value="{{ $level->id }}" {{ old('level_id', $moduleSet->level_id) == $level->id ? 'selected' : '' }}>{{ $level->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="category_id" id="category_id" class="form-select @error('category_id') is-invalid @enderror" required>
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_id', $moduleSet->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="test_type_id" id="test_type_id" class="form-select @error('test_type_id') is-invalid @enderror" required>
                                ...
                                @foreach($testTypes as $type)
                                    <option value="{{ $type->id }}" {{ old('test_type_id', $moduleSet->test_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="form-check form-switch mb-4">
                            <input class="form-check-input" type="checkbox" name="status" value="active" id="statusSwitch" {{ old('status', $moduleSet->status) == 'active' ? 'checked' : '' }}>
                    ...
                    <div class="mt-5 d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-primary px-4" style="border-radius: 10px;">Update Module Portfolio</button>
                    </div>
                </form>
            </div>
        </div>
