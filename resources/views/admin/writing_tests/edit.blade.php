            ...
            <div class="card-body p-4">
                ...
                <div class="mb-4 p-4 border rounded shadow-sm bg-light bg-opacity-50">
                    ...
                    <form action="{{ route('admin.writing-tasks.update', $task) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-muted">Min Word Goal</label>
                                <input type="number" name="min_words" class="form-control form-control-sm" value="{{ old('min_words', $task->min_words ?? ($task->task_number == 1 ? 150 : 250)) }}" placeholder="e.g. 150">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-muted">Max Word Limit (Enforced)</label>
                                <input type="number" name="max_words" class="form-control form-control-sm" value="{{ old('max_words', $task->max_words) }}" placeholder="e.g. 250 (Leave empty for no limit)">
                                <div class="form-text text-muted" style="font-size: 0.72rem;">If set, student cannot write more than this number of words.</div>
                            </div>
                        </div>
